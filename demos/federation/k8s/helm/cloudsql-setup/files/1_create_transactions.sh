...
# Added DB_HOST as we need to connect to it
DB_HOST="${DB_HOST:-localhost}"

add_transaction() {
    DATE=$(date -u +"%Y-%m-%d %H:%M:%S.%3N%z" --date="@$(($6))")
    echo "adding demo transaction: $1 -> $2"
    PGPASSWORD="$POSTGRES_PASSWORD" psql -h "$DB_HOST" -X -v ON_ERROR_STOP=1 -v fromacct="$1" -v toacct="$2" -v fromroute="$3" -v toroute="$4" -v amount="$5" --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    for i in $(seq 1 $PAY_PERIODS); do
        ...
        for p in $(seq 1 $TRANSACTIONS_PER_PERIOD); do
            ...
            if [[ $SENDER_ACCOUNT == "$RECIPIENT_ACCOUNT" ]]; then
                RECIPIENT_ACCOUNT=$(shuf -i 10000-99999 -n1)$(shuf -i 10000-99999 -n1)
            fi
