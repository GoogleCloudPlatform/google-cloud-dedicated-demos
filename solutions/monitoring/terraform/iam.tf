...
resource "google_service_account_iam_member" "mimir_storage_ksa_workload_identity" {
  service_account_id = google_service_account.mimir_storage_sa.name
  role               = "roles/iam.workloadIdentityUser"
  member             = "serviceAccount:${local.wif_name_for_universe}.svc.id.goog[${local.config.k8s_namespace}/mimir-ksa]"
}

# Loki
resource "google_service_account" "loki_storage_sa" {
  project      = data.google_client_config.default.project
  account_id   = local.config.loki_sa
  display_name = "Loki Storage SA"
}

resource "google_project_iam_member" "loki_storage_access" {
  project = data.google_client_config.default.project
  role    = "roles/monitoring.viewer"
...
resource "google_service_account_iam_member" "loki_storage_ksa_workload_identity" {
  service_account_id = google_service_account.loki_storage_sa.name
  role               = "roles/iam.workloadIdentityUser"
  member             = "serviceAccount:${local.wif_name_for_universe}.svc.id.goog[${local.config.k8s_namespace}/loki-ksa]"
}
...
resource "google_storage_bucket_iam_member" "logging_vm_bucket_viewer" {
  count  = local.config.gce_enabled ? 1 : 0
  bucket = local.config.storage_bucket_name
