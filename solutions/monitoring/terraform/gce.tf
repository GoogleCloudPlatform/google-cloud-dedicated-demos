...
resource "google_compute_instance" "logging_demo_vm" {
  ...
  network_interface {
    subnetwork = length(google_compute_subnetwork.monitoring_subnet) > 0 ? google_compute_subnetwork.monitoring_subnet[0].id : "projects/${data.google_project.default.project_id}/regions/${local.config.region}/subnetworks/${local.config.subnet_name}"
  }
  ...
  depends_on = [
    ...
    google_storage_bucket_iam_member.logging_vm_bucket_viewer
  ]
}
