...
**LLM**      | Gemma      | Open-weight model for claim verification & chatbot.
...
4. Navigate to `terraform`, copy `terraform.tfvars.example` to
   `terraform.tfvars`, and update the file with your **mandatory**
   project-specific values:
    ...
    `claims_document_bucket_name` | **Mandatory** | Unique GCS bucket name.
    `hugging_face_token` | **Mandatory** | Hugging Face Access Token (Read).
    `jupyter_password` | **Mandatory** | JupyterLab password.
    `app_login_user` | **Mandatory** | Showroom username (default: `admin`).
    `app_login_password` | **Mandatory** | Web Showroom Dashboard password.
