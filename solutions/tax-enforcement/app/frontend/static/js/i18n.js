 ...
 */
// Frontend Internationalization Engine
window.i18n = {
  dictionary: {},
  currentLang: "en",
  t: function (key) {
...
document.addEventListener("DOMContentLoaded", async function () {
  // Determine stored or default language
...
async function setLanguage(lang) {
  ...
  document.cookie = `ui_lang=${encodeURIComponent(lang)}; path=/; max-age=31536000`;
...
async function loadAndApplyLanguage(lang) {
  try {
    const res = await fetch(`/static/i18n/${lang}.json?t=${Date.now()}`);
    if (res.ok) {
