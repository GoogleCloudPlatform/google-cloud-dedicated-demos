...
class TaxDataGenerator:
  """Generates synthetic tax office data for training and new filings."""
  ...
  def generate_declaration_id(
    self, taxpayer_id: str, year: int, has_crypto: bool = False
  ) -> str:
  ...
  def calculate_derived_fields(self, row: Dict[str, Any]) -> Dict[str, Any]:
  ...
  def determine_anomaly(self, row: Dict[str, Any]) -> Dict[str, Any]:
  ...
  def generate_individual_record(
    self, year: int, data_source: str, force_non_crypto_anomaly: bool = False
  ) -> Dict[str, Any]:
    ...
    income_bracket = random.choices(
      ['low', 'medium', 'high', 'very_high'], weights=[40, 35, 20, 5]
    )[0]
  ...
  def generate_business_record(
    ...
    income_bracket = random.choices(
      ['small', 'medium', 'large', 'enterprise'], weights=[50, 30, 15, 5]
    )[0]
    ...
    if data_source == 'TRAINING':
      ...
      # NEW_FILING: 8% have crypto (more realistic for general business
      # population)
  ...
  def generate_dataset(
    self, training_rows: int, new_filing_rows: int, year: int = 2023
  ) -> pd.DataFrame:
    ...
    target_non_crypto_anomalies = max(
      5, int(new_filing_rows * 0.3)
    )  # At least 30% should be non-crypto anomalies
    ...
    for i in range(new_filing_rows):
      ...
      force_non_crypto = (
        non_crypto_anomaly_count < target_non_crypto_anomalies and random.random() < 0.4
      )  # 40% chance when below target

      # 70% individuals, 30% businesses
      if random.random() < 0.7:
        record = self.generate_individual_record(
          year, 'NEW_FILING', force_non_crypto_anomaly=force_non_crypto
        )
      else:
        record = self.generate_business_record(
          year, 'NEW_FILING', force_non_crypto_anomaly=force_non_crypto
        )
...
def main():
  ...
  parser.add_argument('--year', type=int, default=2023, help='Tax year for the data')
  parser.add_argument(
    '--output',
    type=str,
    required=True,
  ...
  print(f'Anomalies: {len(df[df["is_anomaly"]])}')
  print(f'Anomaly rate: {len(df[df["is_anomaly"]]) / len(df) * 100:.1f}%')
  ...
  print(
    f'  Crypto account holders: {len(df[df["has_crypto_account"]])}'
    f' ({len(df[df["has_crypto_account"]]) / len(df) * 100:.1f}%)'
  )
  print(f'  Crypto non-declarants: {len(df[df["is_crypto_non_declarant"]])}')
  ...
  print(
    '  With crypto:'
    f' {len(training_df[training_df["has_crypto_account"]])}'
    f' ({len(training_df[training_df["has_crypto_account"]]) / len(training_df) * 100:.1f}%)'
  )
  ...
  print(
    '  With crypto:'
    f' {len(new_filing_df[new_filing_df["has_crypto_account"]])}'
    f' ({len(new_filing_df[new_filing_df["has_crypto_account"]]) / len(new_filing_df) * 100:.1f}%)'
  )

  # NEW_FILING anomaly breakdown
  new_filing_anomalies = new_filing_df[new_filing_df['is_anomaly']]
  crypto_anomalies = new_filing_anomalies[new_filing_anomalies['has_crypto_account']]
  non_crypto_anomalies = new_filing_anomalies[
    new_filing_anomalies['has_crypto_account'] == False
  ]

  print('\nNEW_FILING Anomalies Breakdown:')
  print(
    f'  Total anomalies: {len(new_filing_anomalies)}'
    f' ({len(new_filing_anomalies) / len(new_filing_df) * 100:.1f}%)'
  )
  print(
    f'  Crypto-related: {len(crypto_anomalies)}'
    f' ({len(crypto_anomalies) / len(new_filing_anomalies) * 100:.1f}% of'
    ' anomalies)'
  )
  print(
    f'  Non-crypto: {len(non_crypto_anomalies)}'
    f' ({len(non_crypto_anomalies) / len(new_filing_anomalies) * 100:.1f}% of'
    ' anomalies)'
  )

  # Show crypto filings details
  crypto_new_filings = new_filing_df[new_filing_df['has_crypto_account']]

  if len(crypto_new_filings) > 0:
    print('\nCrypto NEW_FILING Declaration IDs:')
    for _, row in crypto_new_filings.iterrows():
      anomaly_indicator = ' [ANOMALY]' if row['is_anomaly'] else ''
      print(
        f'  - {row["declaration_id"]} (TaxpayerID: {row["taxpayer_id"]},'
        f' Type: {row["taxpayer_type"]}, Exchange:'
        f' {row["crypto_exchange_name"]}){anomaly_indicator}'
      )

  # Show non-crypto anomaly examples
  if len(non_crypto_anomalies) > 0:
    print('\nNon-Crypto NEW_FILING Anomalies (sample):')
    for _, row in non_crypto_anomalies.head(10).iterrows():
      print(
        f'  - {row["declaration_id"]} (TaxpayerID: {row["taxpayer_id"]},'
        f' Type: {row["taxpayer_type"]}, Reasons: {row["anomaly_type"]})'
      )
